...
suite.addBatch({
  "join": {
    "the returned hashtable has true for junction points": function() {
      ...
      assert.isTrue(junctionByPoint.get([1, 0]));
    },
    "the returned hashtable has undefined for non-junction points": function() {
      ...
      assert.isUndefined(junctionByPoint.get([0, 0]));
    },
    "exact duplicate lines ABC & ABC have no junctions": function() {
      var junctionByPoint = join(linearize({
    ...
    "reversed duplicate lines ABC & CBA have no junctions": function() {
      var junctionByPoint = join(linearize({
        abc: {type: "LineString", coordinates: [[0, 0], [1, 0], [2, 0]]},
        cba: {type: "LineString", coordinates: [[2, 0], [1, 0], [0, 0]]}
      }));
    ...
    "when a new line DEC shares its end with an old line ABC, there is a junction at C": function() {
      ...
      assert.deepEqual(junctionByPoint.keys(), [[2, 0]]);
    },
    ...
    "when a new line ABDE skips a point with an old line ABCDE, there is a junction at B and D": function() {
      ...
      assert.deepEqual(junctionByPoint.keys(), [[1, 0], [3, 0]]);
    },
    ...
    "when an line ABCDBE self-intersects with its middle, there is a junction at B": function() { // TODO ignore self-intersection?
      var junctionByPoint = join(linearize({
        abcdbe: {type: "LineString", coordinates: [[0, 0], [1, 0], [2, 0], [3, 0], [1, 0], [4, 0]]}
      }));
      assert.deepEqual(junctionByPoint.keys(), [[1, 0]]);
    },
    "when an line ABACD self-intersects with its start, there is a junction at A": function() {
      var junctionByPoint = join(linearize({
        abacd: {type: "LineString", coordinates: [[0, 0], [1, 0], [0, 0], [3, 0], [4, 0]]}
      }));
      assert.deepEqual(junctionByPoint.keys(), [[0, 0]]);
    },
    "when an line ABCDBE self-intersects with its end, there is a junction at E": function() {
      var junctionByPoint = join(linearize({
        abcdbe: {type: "LineString", coordinates: [[0, 0], [1, 0], [4, 0], [3, 0], [4, 0]]}
      }));
      assert.deepEqual(junctionByPoint.keys(), [[4, 0]]);
    },
