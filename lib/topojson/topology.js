...
module.exports = function(objects, options) {
  ...
  function line(points, open) {
    ...
    for (var i = 0; i < n; ++i) { // TODO n + 1 here?
      ...
      // if (a[a.length - 1] !== j) // TODO skip duplicate points during quantization
      // console.warn(i, k, n, a[a.length - 1], point);
      a.push(point);
      t = p;
    }
  ...
  each({
    MultiPoint: function(multiPoint) {
      var points = multiPoint.coordinates, point, i = -1, n = points.length;
      while (++i < n) {
        point = points[i];
  ...
  return {
    ...
    arcs: arcs.map(function(arc) {
      var i = 0,
          n = arc.length,
          point = arc[0],
          x1 = Math.round((point[0] - x0) * kx), dx,
          y1 = Math.round((point[1] - y0) * ky), dy,
          points = [[x1, y1]];
      while (++i < n) {
        point = arc[i];
        x2 = Math.round((point[0] - x0) * kx);
        y2 = Math.round((point[1] - y0) * ky);
        dx = x2 - x1;
        dy = y2 - y1;
        if (dx || dy) {
          points.push([dx, dy]);
