...
module.exports = function(objects, options) {
  ...
  function fix(point) { return [fixX(point[0]), fixY(point[1])]; }
  function fixX(x) { return Math.round((x - x0) * kx); }
  function fixY(y) { return Math.round((y - y0) * ky); }

  function unfix(point) { return [unfixX(point[0]), unfixY(point[1])]; }
  function unfixX(x) { return x / kx + x0; }
  function unfixY(y) { return y / ky + y0; }
  ...
  each({
    ...
    polygon: function(coordinates) {
      ...
      while (++i < n) {
        ring = coordinates[i];
        if (Math.abs(a = area(ring.map(fix).map(unfix))) < ε2) ring.length = 0;
  ...
  objects = each({
    ...
    MultiPoint: function(multiPoint) {
      multiPoint.coordinates = multiPoint.coordinates.map(fix);
    },

    Point: function(point) {
      point.coordinates = fix(point.coordinates);
    },
