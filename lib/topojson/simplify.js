...
module.exports = function(topology, options) {
  ...
  if (options["retain-proportion"]) {
    var areas = [], k = +options["retain-proportion"];
    topology.arcs.forEach(function(arc) {
      arc.forEach(function(point) {
        areas.push(point.area);
      });
    });
    minArea = areas.sort(function(a, b) { return b - a; })[Math.ceil((N - 1) * k)];
  }
  ...
  if (options.verbose) console.warn("simplification: retained " + M + " / " + N + " points (" + Math.round((M / N) * 100) + "%)")
