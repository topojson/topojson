...
function area(t) {
  var a = distance(t[0], t[1]),
      b = distance(t[1], t[2]),
      c = distance(t[2], t[0]);
  return 4 * Math.atan(Math.sqrt(Math.tan((a + b + c) / 2) * Math.tan((b + c) / 2) * Math.tan((a + c) / 2) * Math.tan((a + b) / 2)));
}
...
function distance(p0, p1) {
  var x0 = p0[0] * radians,
      ...
      x1 = p1[0] * radians,
      y1 = p1[1] * radians;
  return Math.acos(Math.max(-1, Math.min(1, Math.sin(y0) * Math.sin(y1) + Math.cos(y0) * Math.cos(y1) * Math.cos(x1 - x0))));
}
