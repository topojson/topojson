...
module.exports = function(objects) {
    ...
    lineEnd: function() {
      if (currentLineStart < index) {
        var j = currentLineStart << 1,
            i = index++ << 1;
        arcifiedPoints[i] = arcifiedPoints[j];
        arcifiedPoints[i + 1] = arcifiedPoints[j + 1];
        var arc = {start: currentLineStart, end: index, next: null};
        while (currentLineStart < index) recordOccurrence(arc, currentLineStart++);
        currentLines.push(arc);
      }
  ...
  var countInPolygon = 0, countSink = {
    polygonStart: function() { countInPolygon = 1; },
    polygonEnd: function() { countInPolygon = 0; },
    lineStart: noop,
    lineEnd: function() { count += countInPolygon; },
    point: function() { ++count; }
  };
