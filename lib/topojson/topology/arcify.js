...
module.exports = function(objects, transform) {
  ...
  var arcifyPolygon = {
    ...
    lineEnd: function() {
      if (currentLineStart < index) {
        arcifiedCoordinates[index++] = arcifiedCoordinates[currentLineStart]; // copy closing coordinate
        var arc = {start: currentLineStart, end: index - 1, next: null};
        arcs.push(arc);
        currentLines.push(arc);
      }
  ...
  var index = 0,
      arcs = [],
      arcifiedCoordinates = new Array(count),
      arcifiedObjects = {};
