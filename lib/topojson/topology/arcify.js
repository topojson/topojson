...
module.exports = function(objects) {
  ...
  function arcifyGeometry(geometry) {
    if (geometry.type === "GeometryCollection") {
      ...
      d3.geo.stream(geometry, round(sink));
      sink.apply(geometry);
    }
    return geometry;
  }
  ...
  var arcifyLine = {
    ...
    point: function(x, y) {
      if (index === currentLineStart || x !== x0 || y !== y0) {
        var i = index++ << 1;
        arcifiedCoordinates[i] = x0 = x;
        arcifiedCoordinates[i + 1] = y0 = y;
      }
    },
    lineEnd: function() {
...
var round = d3.geo.transform({
  point: function(x, y) {
    this.stream.point(Math.round(x * 1e8) / 1e8, Math.round(y * 1e8) / 1e8);
  }
}).stream;
