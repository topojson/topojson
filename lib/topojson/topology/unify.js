...
module.exports = function(objects, transform) {
  ...
  // If the arc can be rotated to the split point at the specified index,
  // then we don’t need to cut it.
  // Returns true if the arc was rotated, or false if split.
  function rotateOrSplit(arc, mid) {
    return rotate(arc, coordinates[mid]) ? true
        : (split(arc, mid), false);
  }
