  ...
  topology.arcs.forEach(function(arc) {
    ...
    if (coarcs = arcsByEnd.get(start)) {
      ...
      while (++coarcIndex < coarcCount) {
        ...
        if (deviation === arc.end) { // old equals or extends new arc
          if (coarc.end - coarc.start !== arc.end - arc.start) { // extends, so split old arc
            ...
            coarc.end = split;
            // if (arcByMid.get(splitPoint) !== coarc) throw new Error("expected coarc to own midpoint");
            // if (arcsByEnd.get(splitPoint) != null) throw new Error("split point shouldn’t have terminating arcs");
            arcByMid.remove(point(split));
            arcsByEnd.set(splitPoint, [arc, coarc.next, coarc]);
          }
