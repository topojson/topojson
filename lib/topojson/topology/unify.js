...
module.exports = function(objects) {
  ...
  unify: while (arc = arcQueue.pop()) {
    ...
    while (arcIndex !== arcEnd) {
      ...
      if (!equalPoint(start, arcPoint) && arcsByEnd.get(arcPoint)) {
  ...
  function split(arc, index) {
    ...
    // If this arc is closed, then add the new arc at the old end point.
    if (equalPoint(arc.start, next.end)) {
      arcsByEnd.get(point(next.end)).push(next);
    }
  ...
  function rotate(arc, newStartPoint) {
    ...
    if (equalPoint(oldStartPoint, point(arc.end)) && (!coarcs || coarcs.length < 2)) {
