...
module.exports = function(objects, transform) {
    ...
    if (coarc = arcByMid.get(startPoint)) {
      ...
      if (!rotate(coarc, startPoint)) {
        ...
        var costep = coarc.start < coarc.end ? 1 : -1,
            ...
            coend = coarc.end;
        while (coindex !== coend) {
          if (equalPoint(coordinates[coindex], startPoint)) {
            split(coarc, coindex);
            coarc = coarc.next;
          }
          coindex += costep;
    ...
    if (coarcs = arcsByEnd.get(startPoint)) {
      ...
      while (++coindex < cocount) {
        ...
        if (deviation === end) {
          ...
          if (Math.abs(coarc.end - coarc.start) > end - start) {
            rotateOrSplit(coarc, (deviation - start) * (coarc.start < coarc.end ? 1 : -1) + coarc.start);
          }
        ...
        if (deviation > start) {
          ...
          if (Math.abs(coarc.end - coarc.start) > deviation - start) {
            if (rotateOrSplit(coarc, (deviation - start) * (coarc.start < coarc.end ? 1 : -1) + coarc.start)) {
              ...
              if (rotate(arc, coordinates[deviation])) {
                arcQueue.push(arc);
                continue unify;
              }
            }
        ...
        if (deviation === end) {
          ...
          if (Math.abs(coarc.end - coarc.start) > end - start) {
            if (!rotateOrSplit(coarc, coarc.end - (deviation - start) * (coarc.start < coarc.end ? 1 : -1))) {
              coarc = coarc.next;
            }
          }
      ...
      if ((coarc = arcByMid.get(midPoint)) && coarc !== arc) {
        ...
        if (!rotate(coarc, midPoint)) {
          ...
          var costep = coarc.start < coarc.end ? 1 : -1,
              ...
              coend = coarc.end;
          while (coindex !== coend) {
            if (equalPoint(coordinates[coindex], midPoint)) {
              split(coarc, coindex);
              coarc = coarc.next;
            }
            coindex += costep;
  ...
  function rotateOrSplit(arc, index) {
    return rotate(arc, coordinates[index]) ? true
        : (split(arc, index), false);
  }

  // Splits the specified arc at the specified index.
  function split(arc, index) {
    var arcs = instancesByArc.get(arc),
        start = coordinates[arc.start],
        mid = coordinates[index],
        end = coordinates[arc.end],
    ...
    arcs = arcsByEnd.get(mid);
    if (arcs) arcs.push(arc, next);
    else arcsByEnd.set(mid, [arc, next]);

    // When an old arc is split,
    // we must update the arcByMid records to point to the new chunk.
    var i = next.start, j = next.end;
    if (i < j) { while (++i < j) { arcByMid.set(coordinates[i], next); } }
    else { while (++j < i) { arcByMid.set(coordinates[j], next); } }
  }
