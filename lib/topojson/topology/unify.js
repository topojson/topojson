...
module.exports = function(objects, transform) {
  ...
  unify: while (arc = arcQueue.pop()) {
    ...
    while (arcIndex !== arcEnd) {
      ...
      if ((coarc = arcByMid.get(arcPoint)) && coarc !== arc) {
        ...
        if (!rotate(coarc, arcPoint)) {
          ...
          var costep = coarc.start < coarc.end ? 1 : -1,
              ...
              coend = coarc.end;
          while (coindex !== coend) {
            if (equalPoint(coordinates[coindex], arcPoint)) {
              split(coarc, coindex);
              coarc = coarc.next;
            }
            coindex += costep;
      ...
      if (!equalPoint(startPoint, arcPoint) && (coarcs = arcsByEnd.get(arcPoint))) {
...
function reverse(array, start, end) {
  for (var middle = start + ((end-- - start) >> 1), t; start < middle; ++start, --end) {
