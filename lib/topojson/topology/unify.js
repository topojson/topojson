...
module.exports = function(objects, transform) {
  ...
  unify: while (arc = arcQueue.pop()) {
    ...
    if (coarc = arcByMid.get(startPoint)) {
      ...
      // then we don’t need to cut the old arc.
      if (rotateable(coarc)) {
        rotate(coarc, startPoint);
      }

      // Otherwise, find where the arc intersects the start, and cut it there.
      ...
      while (++coindex < cocount) {
        ...
        if (deviation > start) {
          ...
          if (coarc.end - coarc.start > deviation - start) {
            ...
            // If we can rotate both the new arc and the old arc
            // to start at the deviation point,
            // then we might be able to avoid an additional split.
            if (closed(arc) && rotateable(coarc)) {
              var deviationPoint = coordinates[deviation];
              rotate(arc, deviationPoint);
    ...
    while (++mid < end) {
      ...
      if ((coarc = arcByMid.get(midPoint)) && coarc !== arc) {
        ...
        // then we don’t need to cut the old arc.
        if (rotateable(coarc)) {
          rotate(coarc, midPoint);
        }

        // Otherwise, find where the arc intersects the point, and cut it there.
        // A self-intersecting arc may require being cut multiple times.
        else {
          var comid = coarc.start,
              coend = coarc.end;
  ...
  // Returns true if the specified arc is closed.
  function closed(arc) {
    return equalPoint(coordinates[arc.start], coordinates[arc.end]);
  }

  // Returns true if the specified arc is closed
  // and doesn’t share endpoints with any other arc,
  // and thus is free to be rotated.
  function rotateable(arc) {
    return closed(arc) && !arcsByEnd.get(coordinates[arc.start]).some(function(coarc) { return coarc !== arc; });
  }

  // Rotates the specified arc to start at the specified point.
  // Requires that the arc is closed,
  // and if the arc is registered,
  // it must not share endpoints with another arc!
  // Also, the arc must contain the specified new start point.
  function rotate(arc, newStartPoint) {
    ...
    if (arcsByEnd.get(oldStartPoint, empty).some(function(coarc) { return coarc === arc; })) {
