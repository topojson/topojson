...
module.exports = function(objects) {
  ...
  function linearizeGeometry(geometry) {
    if (!geometry) return;
    if (geometry.type === "GeometryCollection") return void geometry.geometries.forEach(linearizeGeometry);
    if (linearizeGeometryType.hasOwnProperty(geometry.type)) linearizeGeometryType[geometry.type](geometry);
  }
  ...
  function linearizeLine(line) {
    for (var i = 0, n = line.length; i < n; ++i, ++index) coordinates.push(line[i]);
    var arc = {0: index - n, 1: index - 1};
    lines.push(arc);
    return arc;
  }

  function linearizeRing(ring) {
    for (var i = 0, n = ring.length; i < n; ++i, ++index) coordinates.push(ring[i]);
    var arc = {0: index - n, 1: index - 1};
    rings.push(arc);
    return arc;
  }
