...
module.exports = function(topology) {
  var coordinates = topology.coordinates,
      ...
      visitedByPoint,
      leftByIndex = hashtable(coordinates.length * 1.4, hash, equal, Int32Array, -1, Int32Array, -1),
      rightByIndex = hashtable(coordinates.length * 1.4, hash, equal, Int32Array, -1, Int32Array, -1),
      junctionByIndex = hashtable(coordinates.length * 1.4, hash, equal, Int32Array, -1, Int8Array, 0);
  ...
  for (var i = 0, n = lines.length; i < n; ++i) {
    var line = lines[i],
        ...
        nextIndex = ++lineStart;
    visitedByIndex = hashtable((lineEnd - lineStart) * 1.4, hash, equal, Int32Array, -1, Int8Array, 0);
    junctionByIndex.set(currentIndex, 1); // start
    while (++lineStart <= lineEnd) {
      sequence(previousIndex = currentIndex, currentIndex = nextIndex, nextIndex = lineStart);
    }
    junctionByIndex.set(nextIndex, 1); // end
  }
  ...
  for (var i = 0, n = rings.length; i < n; ++i) {
    var ring = rings[i],
        ...
        nextIndex = ringStart;
    visitedByIndex = hashtable((ringEnd - ringStart + 1) * 1.4, hash, equal, Int32Array, -1, Int8Array, 0);
  ...
  function sequence(previousIndex, currentIndex, nextIndex) {
    ...
    if (leftIndex >= 0) {
      ...
      if (!(equal(leftIndex, previousIndex)
        ...
        junctionByIndex.set(currentIndex, 1);
      }
    } else {
  ...
  var junctions = junctionByIndex.keys(),
      junctionCount = junctions.length,
      junctionByPoint = hashtable(junctionCount * 1.4, hashPoint, equalPoint);
  for (var i = 0; i < junctionCount; ++i) junctionByPoint.set(coordinates[junctions[i]], true);
