...
module.exports = function(topology) {
  ...
  for (var i = 0, n = lines.length; i < n; ++i) {
    ...
    while (++lineStart <= lineEnd) {
      sequence(previousPoint = currentPoint, currentPoint = nextPoint, nextPoint = coordinates[lineStart], true);
    }
  ...
  function sequence(previousPoint, currentPoint, nextPoint, intersectSelf) {
    if (intersectSelf && visitedByPoint.get(currentPoint)) return; // ignore self-intersection
