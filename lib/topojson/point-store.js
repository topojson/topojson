...
function PointStore(objects) {
  ...
  each({
    line: function(line) {
      ...
      for (var i = 0; i <= n; ++i) {
        ...
        if (a[a.length - 1] !== j) a.push(j); // TODO verify
        t = p;
      }
  ...
  function arc(a) {
    ...
    if (n > 2) {
      var i = a[0],
          j = a[n - 1],
          startArcs = arcsByIndex[i],
          endArcs = arcsByIndex[j];
      if (startArcs && startArcs.some(matchForward) || endArcs && endArcs.some(matchReverse)) return;
      if (startArcs) startArcs.push(a); else arcsByIndex[i] = startArcs = [a];
      if (endArcs) endArcs.push(a); else arcsByIndex[j] = endArcs = [a];
      arcs.push(a);
    }
